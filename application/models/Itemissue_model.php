...
class Itemissue_model extends MY_Model {
    ...
    public function getdeliverylist() {
        ...
        $sql="select delivery.*,item.name as `item_name`,item.item_category_id,item_category.item_category ,staff.employee_id,staff.name as staff_name,staff.surname,roles.name from delivery inner join item on item.id=delivery.item_id inner join item_category on item_category.id=item.item_category_id inner join staff on staff.id=delivery.issue_to inner join staff_roles on staff_roles.staff_id =staff.id inner join roles on roles.id= staff_roles.role_id ";
        $this->datatables->query($sql)
            ->orderable('item.name,item_category,issue_date,staff.name,issue_by,quantity,null')
            ->searchable('item.name,item_category,issue_date,staff.name,issue_by,delivery.quantity,null')
            ->query_where_enable(TRUE);
        return $this->datatables->generate('json');
    }
    ...
    public function adds($data) {
        ...
        if (isset($data['id'])) {
            ...
            $message = UPDATE_RECORD_CONSTANT . " On  delivery id " . $data['id'];
            $action = "Update";
            $record_id = $data['id'];
            $this->log($message, $record_id, $action);
            //======================Code End==============================
            ...
            $insert_id = $this->db->insert_id();
            $message = INSERT_RECORD_CONSTANT . " On delivery id " . $insert_id;
            $action = "Insert";
            $record_id = $insert_id;
            $this->log($message, $record_id, $action);
            //======================Code End==============================
